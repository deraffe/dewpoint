...
def saturation_vapour_pressure(temperature: float) -> float:
    svp = alpha * math.pow(math.e, (beta * temperature) / (lambda_ + temperature))
...
def dewpoint_1(temperature: float, relative_humidity: float) -> float:
    ...
    dp1 = (lambda_ * math.log(vp_over_alpha)) / (beta - math.log(vp_over_alpha))
    log.debug(f"{dp1=}°C")
    return dp1
...
def dewpoint_2(temperature: float, relative_humidity: float) -> float:
    dp_zeroeth_part = math.log(relative_humidity / 100) + (
        (beta * temperature) / (lambda_ + temperature)
    )
...
def temperature(td: float, rh: float) -> float:
    ...
    log.debug(f"saturation vapour pressure for wanted temperature: {pst:.2f}hPA")
...
def main():
    ...
    parser.add_argument(
        "-t", "--temperature", type=float, help="temperature in degrees celsius"
    )
    parser.add_argument(
        "-r", "--relative_humidity", type=float, help="relative humidity in percent"
    )
    parser.add_argument(
    ...
    if t is not None and rh is not None:
        ...
        print(f"The relative humidity at {t}°C and {td}°C dewpoint is {rhd:.2%}")
